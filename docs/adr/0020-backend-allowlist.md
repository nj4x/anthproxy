...
4. **Validation is fail-fast, at parse time; advisory tokens document intent without gating enablement.** `--backends` is parsed as: split on commas, strip whitespace from each token, drop empty tokens, de-duplicate (preserving first occurrence). The result is validated against the full discovered `backend_names()` plus `internal_backend_names()`; unknown tokens and an empty resulting set are `p.error()`. Internal backends like `oauth` are accepted as **advisory tokens** — they document operator intent in the command line but do not affect enablement, which is governed by `_is_enabled()` exemptions. A token listed in `--backends` is stored in the allowlist set and passed through to `set_enabled_backends()` unchanged — it is *not* filtered out before that call. It is inert there because `_is_enabled()` checks `internal_backend_names()` membership first and short-circuits to always-enabled before ever consulting the installed allowlist, so `oauth`'s presence or absence in the stored set has no observable effect: `oauth` remains always-enabled regardless of whether it appears in the allowlist string. The CLI flag takes precedence over the `ANTHPROXY_BACKENDS` environment variable; the env var is only consulted when the flag is absent. `--backend` loses its argparse `choices=` (which cannot express a dependency on another flag). The ordering is: `--backends` tokens are validated against the unfiltered `backend_names()` plus internal backends (discovery has already run, the filter has not yet been installed); `set_enabled_backends()` then installs the filter, still containing any advisory tokens; `--backend` is validated last, against the now-filtered `backend_names()` (which excludes internal backends regardless of what's in the stored filter). Validation of the two flags therefore straddles the install point — `--backends` validates before it, `--backend` validates after it. No caller downstream of `set_enabled_backends()` can obtain the full discovered set.
